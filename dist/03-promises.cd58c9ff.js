...
function submitCreatePromises(e) {
    e.preventDefault();
    let delay1 = firstDelayMs.valueAsNumber;
    ...
    for(let i = 1; i <= amountVal; i += 1){
        createPromise(i, delay1).then(({ position , delay  })=>{
            (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${i} in ${delay}ms`);
        }).catch(({ position , delay  })=>{
            (0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${i} in ${delay}ms`);
        });
        delay1 += delayStepMsVal;
    }
}
...
},{}],"jvRD8":[function() {},{}],"gkKU3":[function(require,module,exports) {
